...
const getAllTrips = async (request, response) => {
  try {
    ...
    const results = await pool.query(
      "SELECT t.* FROM trips_destinations td, trips t WHERE td.trip_id = t.id AND td.destination_id = $1",
      [destination_id]
    );
    response.status(200).json(results.rows);
    console.log(results.rows);
...
const getAllDestinations = async (request, response) => {
  try {
    ...
    const results = await pool.query(
      "SELECT d.* FROM trips_destinations td, destinations d WHERE td.destination_id = d.id AND td.trip_id = $1",
      [trip_id]
    );
    response.status(200).json(results.rows);
    console.log(results.rows);
